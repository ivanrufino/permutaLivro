...
class Correio {
	...
	* return void
	*/
	public function __construct($parametros=false){
            $id = $parametros['COD_RASTREIO'];
            if ($id){
	 ...
	 public function array2xml($array, $xml = false){
	    ...
	    foreach($array as $key => $value){
	        if(is_array($value)){
	        	...
	        	if (is_numeric($key)) $key="ocorrencias";
...
//$ca=(array)$c;
$ca = json_decode(json_encode($c), true);
echo $ca['status'];
echo $ca['track'][0]['data'];*/



//echo "Código de entrada: $cod <br>";

$ca = json_decode(json_encode($c), true);
//print_r($ca);
print $c->array2xml($ca);
-->